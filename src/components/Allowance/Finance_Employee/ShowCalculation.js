...
const {allowances,users,employees,config,place,
       fieldEmployees,climatePlaces}=useContext(StoreContext)
        /**calculation class */
const {_id:id,creater:emp_id}=allowance
const calculation=new Calculation(place.state,allowances.state,
                  employees.state,users.state,config.state[0],climatePlaces.state)
...
saveSeen(approveTl)
  ...
  saveSeen(approveDr)
    ...
    return (
     ...
     <FontAwesomeIcon icon={faUserCircle} className='mx-2 text-light' />
                ...
                <div className="col-lg-6">
   {
    ...
    <div className="text-center font-weight-bold card">
            Destination place_name:{Places.findPlace(allowance.place_id)?
                 Places.findPlace(allowance.place_id).name:
                 ''                 
             } <br/>
           Destination place region:{Places.findPlace(allowance.place_id)?
                 Places.findPlace(allowance.place_id).region:
                 allowance.region} <br/>
            Destination place type:{allowance.place_type} <br/>
            climate general place name:{climate.findClimatePlace(allowance.climate_id)?
            climate.findClimatePlace(allowance.climate_id).general_name:''} <br/>
            climate place name:{climate.findClimatePlace(allowance.climate_id)?
            climate.findClimatePlace(allowance.climate_id).name:''} <br/>
            climate place level:{climate.findClimatePlace(allowance.climate_id)?
            climate.findClimatePlace(allowance.climate_id).level:''}  <br/>       
