...
import { faEnvelope, faInfo, faPhone, faCalculator } from '@fortawesome/free-solid-svg-icons'
...
const Navbar=(props)=> {
  ...
  useEffect(() => {
    ...
    //if the user is not authenticated push to login
     ...
     token?x=1:props.history.push('/login')
    let {width}=window.screen
    setState({...state,screen:width})
    if(width<=892){
       togglers({...uiContents,collapse:'closed-sidebar-mobile closed-sidebar'})
    }    
  }, [user_type,token])
    const openSearch=()=>{
    ...
    const openProfile=()=>{
        ...
        let profileOpener=width<=892?(po===''?'header-mobile-open':''):('')
  ...
  let {width}=window.screen
  let toggler=width<=892?(
    collapse==='closed-sidebar-mobile closed-sidebar'?
    'closed-sidebar-mobile closed-sidebar sidebar-mobile-open':'closed-sidebar-mobile closed-sidebar'
  ):(
collapse===''?'closed-sidebar':''
  )
      ...
      <div className={"app-header header-shadow "+headertheme}>
           ...
           <FontAwesomeIcon icon={faCalculator} />
                ...
                <div className="header__pane ml-auto">
                    <div>
                        <button type="button" className={"hamburger close-sidebar-btn hamburger--elastic "+state.toggleButton}
                         data-classname="closed-sidebar" onClick={()=>toggler()}>
            ...
            <div className="app-header__menu">
                <span>
    <button type="button" 
    className={"btn-icon btn-icon-only btn btn-primary btn-sm mobile-toggle-header-nav "+state.acivateProfile}
     onClick={()=>openProfile()}>
...
<div tabIndex="-1" role="menu" aria-hidden="true" className="dropdown-menu dropdown-menu-right">
    <button type="button" tabIndex="0" className="dropdown-item">My Account</button>
    <button type="button" tabIndex="0" className="dropdown-item">Logout</button>
     ...
     {allowance.Department(allowance.getEmp_id())} {allowance.UserRole(allowance.getEmp_id())}
                                    </div>
                                </div>
