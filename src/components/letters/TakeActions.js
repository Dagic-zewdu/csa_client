...
const {socket,letters,employees,messages,users,connections}=useContext(StoreContext)
...
const emp_id=Mess.getEmp_id()
 const handleSubmit=async e=>{
      e.preventDefault()
try{
 setState({...state,...saveProcess('initial','Taking decision please wait ...')})   
   ...
   //update the letter
const req=await axios.put(host+'/letter',{data:encryptObject({...letter,...userInfo()})})    
  const res=decrptObject(req.data)
  if(res.updated){
  let proceed= Mess.proceedStage(id) //checks wheter to proceed this stage
  console.log('proceed-'+proceed)
  if(proceed){  //procedd to the next stage
    ...
    if(next){
        let Data=nextManager.map(m=> {
           ...
           let Req=await axios.post(host+'/messages',{ data:
            encryptObject({messages:[...Data,{
      message:state.approved==='Approved'?'System Notification '+Mess.Name(emp_id)+' has approved your letter':
      state.approved==='unApproved'?'System notification '+Mess.Name(emp_id)+' un approved your letter':'',
                 letter_id:'',
                 file_name:'',
                 sender:emp_id,
                 reciever:Letter.creater
            }],...userInfo()})}) //sending message with system notification
            let Res=decrptObject(Req.data) //decrypting response
         console.log(Data,Res)
            if(Res.created){
        sendToParticipant()   
            }
            else{sendToParticipant()}   
    }
  else{
    sendToParticipant()
  }
...
const M=new Message(messages.state,connections.state,Letters,users.state,employees.state)    
let proceed= M.proceedStage(id) //checks wheter to proceed this stage
if(proceed){  //procedd to the next stage
 let nextManager=M.nextEmployee(id)
 let next=nextManager.length?false:true
 if(next){
...
let req=await axios.post(host+'/messages',{ data:
    encryptObject({messages:[...Part,{
        message:'System Notification- Letter '+ Letter.id+
...
setState({...state,...saveProcess('success','Decison made succesfully')})
}
  ...
  <FontAwesomeIcon icon={faCheck} className='text-info fa-2x mx-2 my-auto '/>
