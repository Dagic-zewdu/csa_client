...
export class AllowanceClass extends EmployeeClass {
    ...
    * commented
    *  */   
      rankProgress=status=>status==='Approved'?4:
        status==='unApproved'?1:
        status==='commented'?2:0
    ...
    if(this.findAllowance(id)===undefined){
      ...
      let {approval_manager,f_pending_dr,f_pending_emp,f_approve_tm,
          f_pending_tl,f_approve_dr}=this.findAllowance(id)
       //step-1 check if the approval manager has seen and approved the allowance
       let ama=approval_manager.seen&&approval_manager.approved==='waiting'?
       3:this.rankProgress(approval_manager.approved)
       i=i+ama
       //step-2 check if the finance director has seen and approve
       let fpd=f_pending_dr.seen&&f_pending_dr.approved==='waiting'?
       3:this.rankProgress(f_pending_dr.approved)
       i=i+fpd
       ...
       let fatl=f_approve_tm.seen&&f_approve_tm.approved==='waiting'?3:
       this.rankProgress(f_approve_tm.approved)
        i=i+fatl 
        //step-6 check if the finance director has seen & approved 
        let fadr=f_approve_dr.seen&&f_approve_dr.approved==='waiting'?
        3:this.rankProgress(f_approve_dr.approved) 
        i=i+fadr
      return i
    }  
    }
    ...
    tellProgress=(id)=>{
      ...
      checkApproval=(Num)=>{
        ...
        else if(Num===3||Num===7||Num===11||Num===17||Num===14){
            return   'Reviewed'  
        }
    ...
    /**Approval manager */
    approve_Allowances=()=>{ //accepts emp_id and return managers approval allowance
        let allowances=this.allowances.filter(f=>{
          return (f.approval_manager.emp_id === this.getEmp_id() && f.save_options==='approve')
      })
      return allowances
    }
     ...
     */
       DirectorPending=()=>{
        let fda=this.allowances.filter(f=>{
          return   f.approval_manager.approved==='Approved' && f.save_options==='approve' && f.approval_manager.seen
        })
        return fda
    }
     seenDirectorPending=()=>{
    ...
    IncomingCalculations=()=>{
      let calculations=this.DirectorPending().filter(c=>{
         return c.f_pending_tl.forwarded && c.f_pending_emp.emp_id === this.getEmp_id() 
      })
      return calculations
   }
...
/**recieves allowance id and returns whether the calculation is done */   
  ...
  let done=this.IncomingCalculations().find(i=>{
    return i._id === id && (i.f_pending_emp.calculated==='approve' || i.f_pending_emp.calculated==='draft')
  })
  ...
  let comment=this.seenCalculations().filter(c=>{
    return this.isCalculationDone(c._id) && c.f_approve_tm.approved === 'commented' && !c.f_pending_emp.redone 
  })
  return comment      
      }
...
/**returns allowance that need to be approved or already approved allwoance */
    ...
    const dr=this.allowances.filter(a=>{
      return a.f_pending_dr.emp_id === this.getEmp_id() && a.f_approve_tm.approved === 'Approved'
    })
    return dr
     }
     NewDrApprove=()=>{
     ...
     /**returns allowance that are commented by director */ 
      drCommented=()=>{
        let comment=this.seenCalculations().filter(c=>{
    return this.isCalculationDone(c._id) && c.f_approve_dr.approved === 'commented' && !c.f_pending_emp.redone_director 
        })
        return comment      
            }
      /**returns redone allowances that are commented by finance director */
  reDone=()=>this.DrApproveAllowance().filter(c=>c.f_pending_emp.redone_director)      
...
allCompleted=()=>this.allowances.filter(a=>
